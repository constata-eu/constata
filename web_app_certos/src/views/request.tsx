...
import SelectRequestState from "../components/select_request_state";
...
function RequestList(props) {
  ...
  const requestFilters = [
    ...
    <SelectRequestState source="stateEq" alwaysOn />,
  ];
  ...
  return (
    <Container maxWidth="md" sx={{mb:3}}>
      <Card>
        <CardTitle text="resources.Request.admin_title"/>
        <List {...props}
          empty={false}
          sort={defaultSort}
        ...
        >
          { isSmall ?
            ...
            <Datagrid bulkActionButtons={false}>
              ...
              <TranslatedTextField source="state" translation="certos.request.states" />
              <FunctionField
                source='templateId'
                render={record => {
...
function RequestShow(props){
  const translate = useTranslate();
  const requestId = useGetRecordId();
  const dataProvider = useDataProvider();
    ...
    <ShowBase {...props} actions={false}>
      <Container maxWidth="md" sx={{mb:3}}>
        <Card sx={{ mb: 3 }}>
          ...
          <Box py={1}>
            <SimpleShowLayout>
              ...
              <WithRecord render={ record => {
                if (record.state !== "completed" && record.state !== "failed") {
                  return (<TranslatedTextField
                    source="state"
                    label="resources.Request.fields.nextStep"
                    translation="resources.Request.fields.nextSteps"
                  />);
                } else {
                  return <></>;
                }
              ...
              <WithRecord render={ record => 
                record.errors && <TranslatedTextField source="errors" translation="certos.request.errors" />
              } />
            </SimpleShowLayout>
          </Box>
        </Card>
        <Card>
          ...
          <WithRecord render={ request => 
            <ReferenceManyField reference="Entry" target="requestIdEq" label=""
                    ...
                    primaryText={(record) => 
                      <Box component="div" sx={{cursor:"pointer", px: 2 }} onClick={() => onEntryClick(record) }>
                        <Typography variant="button" id={`preview-` + record.id}>
                          ...
                          &nbsp;
                          { translate(`certos.wizard.kind.${request.templateKind}`)}
                          &nbsp;
                          #{record.id}
                        </Typography>
...
export {RequestList, RequestShow};
