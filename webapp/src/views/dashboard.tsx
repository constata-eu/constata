...
export default function Dashboard() {
  ...
  if (!ready)
    return (
      <Container maxWidth="md" id="constata_dashboard_loading">
        <Skeleton />
        <Skeleton />
        <Skeleton />
      </Container>
  ...
  return (
    <Container maxWidth="md" id="constata_dashboard">
      <AccountStateSection />
      <Box mb={3}>
        <Head1 sx={{ mb: 2 }}>
          {" "}
          {translate("certos.dashboard.issue.title")}{" "}
        </Head1>
        <Typography>{translate("certos.dashboard.issue.text")}</Typography>
      </Box>
      <EndorsementsSection />
      <EmailAddress />
      <IssuancesSection />
