  ...
  constata_lib::describe_one! {
    ...
    integration_test!{ issues_diplomas_from_csv_and_completes_them (c, d)
      ...
      for _ in 0..4 {
        ...
        let csv = format!("{}/tests/resources/default_recipients.csv", env::current_dir().unwrap().display());
    ...
    integration_test!{ use_previous_template_to_create_issuances (c, d)
      ...
      c.site.issuance().create_all_received().await.expect("to create sucessfully the entries");
    ...
    integration_test!{ uploading_csv_in_wizard (c, d)
      signup(&c, &d, true).await;
      let files = vec![
        ("default_recipients.csv", r"Arte con plastilina*"),
        ("default_recipients_special.csv", r"Arte con plastiliña,*"),
        ("recipients_windows.csv", r"Arte con plastiliña,*"),
        ("recipients_semicolon.csv", r"Arte con plastilina*"),
        ("recipients_semicolon_special.csv", r"Arte con plastiliña;*")
      ];
    ...
    integration_test!{ shows_usage_statistics_for_issuances (c, d)
      ...
      chain.fund_signer_wallet();
      chain.simulate_stamping().await;
      for entry in c.site.issuance().find(&2).await?.entry_vec().await? {
        entry.in_signed()?.try_complete().await?;
      }
    ...
    */
    ...
    async fn add_recipients_with_csv(d: &Selenium, site: &Site, csv: &str) {
      ...
      d.click("#continue").await;
      d.wait_for("span[role='progressbar']").await;
      site.issuance().create_all_received().await.expect("to successfully create the entries");
      d.wait_for_text("#preview_container h2", "Review and sign").await;
    }
    ...
    async fn add_all_recipients(d: &Selenium, site: &Site, recipient_count: i32) {
      ...
      site.issuance().create_all_received().await.expect("to create sucessfully the entries");
      d.wait_for_text("#preview_container h2", "Review and sign").await;
    }
