...
impl KycRequestInput {
  pub async fn process(self, context: &Context) -> FieldResult<KycRequest> {
    ...
    let kyc_request = context.site.kyc_request().insert(InsertKycRequest {
      person_id: *person.id(),
...
#[rocket::async_trait]
impl Showable<db::KycRequest, KycRequestFilter> for KycRequest {
  ...
  async fn db_to_graphql(d: db::KycRequest) -> ConstataResult<Self> {
    let a = d.attrs;
    Ok(KycRequest {
      id: a.id,
