...
#[rocket::async_trait]
impl Showable<db::Template, TemplateFilter> for Template {
  ...
  async fn db_to_graphql(d: db::Template) -> ConstataResult<Self> {
    let mut admin_visited_count = 0;
    let mut entries_count = 0;
    let mut public_visit_count = 0;

    for r in d.issuance_vec().await? {
...
impl TemplateInput {
  pub async fn update_template(&self, context: &Context) -> FieldResult<Template> {
    ...
    Ok(Template::db_to_graphql(template).await?)
  }
}
